...
/--
  The notation `[|...|]` is used to construct a literal of this type.
  see also `[| a ~~ b : c |] = gen a (some b) c`.

  e.g. `[|1,2,3|] = 1 ::' 2 ::' 3 ::' nil`. Note that
  - `[||] = nil` where `nil = (nil : LazyList α)`
  - `[|_,..|] = .. ::' nil` where `nil = (↑nil : Thunk (LazyList α))`.
    Unlike `List`, due to how the tail is implemented (wrapped in a `Thunk`) and
    obivously we cannot match two thunks without computing them first thus
    `[|_,_,..|]` cannot be used in pattern matching.
...
def Array.foldl1M [Inhabited α] [Monad m] (f : α -> α -> m α) (arr : Array α) (start := 0) (stop := arr.size) : m α :=
  loop start arr (stop - start) where
  loop i arr
    | 0 => panic! "empty array"
    | 1 => pure arr[i]!
    | n + 2 =>
      f arr[i]! arr[i + 1]! >>= fun fxy =>
        loop (i + 1) (arr.set! (i + 1) fxy) (n + 1)
...
@[always_inline, inline] protected def pure := @singleton
instance : Pure LazyList := ⟨LazyList.pure⟩
...
instance : Functor LazyList where map
...
def filterMap' (p : α -> Bool) (f : α -> β) : LazyList α -> LazyList β
  | nil => nil
  | x ::' xs =>
    if p x then
      f x ::' filterMap' p f xs.get
    else filterMap' p f xs.get
def getAllSome : LazyList (Option α) -> LazyList α
  | nil => nil
  | .some x ::' xs => x ::' getAllSome xs.get
  | .none ::' xs => getAllSome xs.get
...
private def seq {α β : Type u} (mf : LazyList $ α -> β) (mx : Unit -> LazyList α) : LazyList β :=
  mf >>= (· <$> mx ())
instance : Applicative LazyList where pure := singleton; seq
...
/--
  ...
  let g10 := gen 0 10 1 in
  (· , ·) <$> g10 <*> g10
    |>.filter $ uncurry (· + · = 10) = [(x, y) where x + y = 10
                                        | x <- [|0 ~~ 10|]
                                        , y <- [|0 ~~ 10|]]
...
class Mappable (F : Type u -> Type v) extends Functor F where
  ...
  filterMap (p : α -> Option β) : F α -> F β
  /--
  Usually if there's `filterMap`, there must be a `filter` method as well for collections. But if it doesn't then we define `filter` in terms of `filterMap`.
  But a direct implementation has, very likely, better performance.
  -/
  filter (p : α -> Bool) : F α -> F α := filterMap (fun x => if p x then some x else none)
  map f := filterMap (some ∘ f)
